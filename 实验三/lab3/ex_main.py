...
def preprocessing(img):
    ...
    GaussianBlur_img = cv2.GaussianBlur(histogram_equalization_img, (3, 3), 0)  # 取出噪声
    # canny边缘检测
...
定位车牌号
'''
def locate_license(img, afterimg, number=1):
    # 利用该函数来查找物体的轮廓
    # 该参数接收的参数是二值图
    # 第一个参数是寻找轮廓的图像
    ...
    for c in contours:
        ...
        block.append([r, a, s])
    block = sorted(block, key=lambda b: b[1])[-(number + 3):]
    # 使用颜色识别找出车牌所在的区域
    tmp_ans = []
    for i in range(len(block)):
        ...
        mean = cv2.mean(mask)
        tmp_ans.append([i, mean])
    tmp_ans = sorted(tmp_ans, key=lambda b : b[1])[::-1]
    return block, tmp_ans[:number]


if __name__ == '__main__':
        img = cv2.imread('./pic/6.jpg', cv2.IMREAD_COLOR)
        # 预处理图像
        openingimg = preprocessing(img)
        # 定位车牌的位置
        number = 2
        rect, order = locate_license(openingimg, img, number=number)
        # 将车牌所在的区域，使用矩形框起来
        for j in range(number):
            k = order[j][0]
            cv2.rectangle(img, (rect[k][0][0], rect[k][0][1]), (rect[k][0][2], rect[k][0][3]), (0, 100, 200), 2)
